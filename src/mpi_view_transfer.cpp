...
// ============================================================================
//  OPTIMIZED — replaces mpi_view_transfer.cpp (v2, CudaSpace architecture).
//
//  OPT-T1 (P0, correctness)  setup_Local: the Allgather + 3D-reshape prefix
//         sums decoded ranks as (px + rx*py + rx*ry*pz), but the actual rank
//         linearization is  me = px + rx*pz + rx*rz*py.  Result: y_lo wrong
//         whenever rz>1, z_lo wrong whenever ry>1 — per-rank Taylor-Green
//         phases misaligned and MPI-IO subarray offsets corrupted. Replaced
//         with the closed-form block distribution (zero communication; the
//         three Allgathers, three Barriers and the VLAs disappear).
//
//  OPT-T2 (P0, correctness)  exchange_f fully restored (it was debug-gutted
//         to a single edge with Waitall(1); the live frontleft message used
//         tag/slot 1, colliding with face slot 0 once un-commented). Now
//         loop-driven over the existing face/edge/point tables: 26 channels,
//         tag = channel id, one request slot each, Waitall(26). Self-sends
//         (periodic wrap onto the same rank at small counts) are legal with
//         Isend/Irecv. The stray MPI_Barrier is gone — Waitall is the sync.
//
//  OPT-T3 (P0, performance)  pack_f/unpack_f: Kokkos::deep_copy(dst, src)
//         WITHOUT an execution-space argument is synchronous — it fences the
//         device per call. 52 copies = 52 global fences per step. Passing an
//         execution-space instance makes them asynchronous kernels on the
//         default stream: 52 fences -> 1 (end of pack_f, before MPI).
//
//  OPT-T4  Buffer label copy-paste fixes (m_frontright was labeled
//         "m_frontdown", m_frontout was "m_downout", ... ) — duplicate View
//         labels confuse profiler attribution and Kokkos debug tooling.
//
//  RUNTIME REQUIREMENT: buffers live in CudaSpace and .data() goes straight
//  into MPI — this is GPU-aware MPI territory. On Polaris:
//      module load craype-accel-nvidia80
//      export MPICH_GPU_SUPPORT_ENABLED=1
//  Without it the first Isend segfaults. (This is a feature, not a bug —
//  it's the GPUDirect path.)
// ============================================================================
...
void LBM::setup_Local()
{
    ...
    // OPT-T1: closed-form block distribution. Ranks with coordinate < gl%r
    // own one extra cell, so lo(p) = p*(gl/r) + min(p, gl%r). Identical
    // values to a correct prefix sum, zero communication, and it removes
    // the transposed-index bug of the gather/reshape version.
...
void LBM::setup_f()
{
    // 1-deep halos: streaming pulls only +-1, and the periodic TG case has
    // no wall BCs reading +-2, so single-layer exchange is sufficient.
    // 6 faces
...
{
    // OPT-T3: async deep_copy on the default execution-space instance —
    // 26 copy kernels enqueued in order, ONE fence before MPI touches them.
    Kokkos::DefaultExecutionSpace ex;

    // 6 faces
    LBM_PROF_BEGIN(P_PACK_FACE);
    ...
    LBM_PROF_END(P_PACK_FACE);

    // 12 edges
    LBM_PROF_BEGIN(P_PACK_EDGE);
    ...
    LBM_PROF_END(P_PACK_EDGE);

    // 8 corners
    LBM_PROF_BEGIN(P_PACK_CORNER);
    ...
    LBM_PROF_END(P_PACK_CORNER);

    LBM_PROF_BEGIN(P_PACK_FENCE);
    ex.fence(); // MPI reads these device buffers next
    LBM_PROF_END(P_PACK_FENCE);
}

void LBM::exchange_f()
{
    // OPT-T2: 26 channels, loop-driven off the neighbor tables.
    // Channel c: send my <dir c> boundary to send_rank[c] with tag c,
    // receive the opposite boundary from recv_rank[c] with the same tag —
    // symmetric SPMD, so tags always match. Self-messages (periodic wrap at
    // small rank counts) are NOT handed to MPI -- see the shortcut below.
...
#ifdef LBM_PROFILE
    // Exact halo volume, straight off the buffers MPI is about to ship. Cheap
    // (26 size() calls) and re-registers the same value every step.
    {
        double sb = 0.0, rb = 0.0;
        for (int d = 0; d < 6; ++d)  { sb += (double)fout[d].size(); rb += (double)fin[fopp[d]].size(); }
        for (int d = 0; d < 12; ++d) { sb += (double)eout[d].size(); rb += (double)ein[eopp[d]].size(); }
        for (int d = 0; d < 8; ++d)  { sb += (double)pout[d].size(); rb += (double)pin[popp[d]].size(); }
        LBM_PROF_SET_HALO(sb * sizeof(double), rb * sizeof(double));
    }
#endif

    // SELF-CHANNEL SHORTCUT.
    // When a decomposition axis has extent 1 (e.g. rx=1), the periodic wrap
    // puts both neighbours on THIS rank, and the channel becomes an Isend/Irecv
    // to me. Cray MPICH handles multi-MB GPU self-messages extremely badly, and
    // because they sit in the same Waitall they stall every other channel:
    // measured with mpi_gpu_bw on Polaris, 2 self-sends among 26 channels drop
    // the whole exchange from 91 GB/s to 6.5 GB/s. Removing them recovers 10x.
    //
    // A self-channel needs no MPI at all -- both buffers are local, so it is a
    // device-to-device copy. Enqueued on the default execution-space instance,
    // so it is stream-ordered ahead of unpack_f's reads exactly like the MPI
    // path was.
    Kokkos::DefaultExecutionSpace ex;
    auto post = [&](int ch, const buffer_pack_f &sbuf, const buffer_pack_f &rbuf,
                    int dst, int src) {
        if (dst == me && src == me)
        {
            Kokkos::deep_copy(ex, rbuf, sbuf);
            rs[ch] = MPI_REQUEST_NULL;   // Waitall ignores null requests
            rr[ch] = MPI_REQUEST_NULL;
            return;
        }
        MPI_Isend(sbuf.data(), sbuf.size(), MPI_DOUBLE, dst, ch, comm, &rs[ch]);
        MPI_Irecv(rbuf.data(), rbuf.size(), MPI_DOUBLE, src, ch, comm, &rr[ch]);
    };

    LBM_PROF_BEGIN(P_MPI_POST);
    int ch = 0;
    for (int d = 0; d < 6; ++d, ++ch)
        post(ch, fout[d], fin[fopp[d]], face_send[d], face_recv[d]);
    for (int d = 0; d < 12; ++d, ++ch)
        post(ch, eout[d], ein[eopp[d]], edge_send[d], edge_recv[d]);
    for (int d = 0; d < 8; ++d, ++ch)
        post(ch, pout[d], pin[popp[d]], point_send[d], point_recv[d]);

    LBM_PROF_END(P_MPI_POST);

    LBM_PROF_BEGIN(P_MPI_WAIT);
    MPI_Waitall(26, rr, MPI_STATUSES_IGNORE);
    MPI_Waitall(26, rs, MPI_STATUSES_IGNORE);
    LBM_PROF_END(P_MPI_WAIT);
...
void LBM::unpack_f(buffer_f ff)
{
    ...
    LBM_PROF_END(P_UNPACK_FACE);

    // 12 edges
    LBM_PROF_BEGIN(P_UNPACK_EDGE);
    ...
    LBM_PROF_END(P_UNPACK_EDGE);

    // 8 corners
    LBM_PROF_BEGIN(P_UNPACK_CORNER);
    ...
    LBM_PROF_END(P_UNPACK_CORNER);

#ifdef LBM_PROFILE
    // PROFILE-ONLY fence, absent from every other build. Without it these 26
    // copies complete inside Streaming's fence together with stream1, and the
    // two are indistinguishable in the report — which defeats the purpose of
    // having separate rows. Costs one extra synchronisation per step, so never
    // read throughput off a profile build.
    LBM_PROF_BEGIN(P_UNPACK_FENCE);
    ex.fence();
    LBM_PROF_END(P_UNPACK_FENCE);
#endif

    // No fence: the next consumer (stream1) runs on the same execution-space
    // instance and is therefore ordered after these copies.
}
